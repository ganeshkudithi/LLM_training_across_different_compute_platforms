...
raw_dataset = load_dataset(
    "text",
    data_files="/home/samritm/nanoGPT/finetuning/sample_paragraphs_2000.txt"
)["train"]

def convert_to_orpo(sample):
    paragraph = sample["text"]

    sample["instruction"] = "Rewrite the following paragraph"
    sample["input"] = paragraph

    # Accepted = original paragraph
    sample["accepted"] = paragraph

    # Rejected = a corrupted / shortened version
    sample["rejected"] = paragraph[: len(paragraph)//2]  # simple example

    return sample

dataset = raw_dataset.map(convert_to_orpo)
